...
## `graphrag-query` - 结构化图谱查询（计划、步骤、动作）
涉及到“具体训练指导”时必须使用此类：
- 询问训练动作怎么做（如“深蹲怎么练？”）
- 询问动作要点、步骤、姿势纠正
- 训练计划制定（如“给我一个增肌计划”）
...
## `text2sql-query` - 数据库统计查询（训练数据统计/分析）
涉及健身数据统计、排名、数量、对比时使用此类：
典型场景：
- **统计类**：  
  “有多少个胸部训练动作？”  
  “一天摄入多少蛋白质合适？”
...
**重要提示：**
1. 任何包含“数量 / 统计 / 排名 / 对比 / 平均 / 占比”的问题 → `text2sql-query`
2. 包含“怎么做 / 怎么练 / 要点 / 计划 / 姿势”的问题 → `graphrag-query`
3. 涉及原理、科普、深度解释 → `kb-query`
4. 图像 → `image-query`
5. 上传文档 → `file-query`
6. 若问题模糊、信息不足 → `additional-query`
...
记住：
- **凡是与“动作、训练、计划、营养、器械、怎么练”相关的问题，默认选择 `graphrag-query`。**
